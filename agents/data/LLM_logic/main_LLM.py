...
def create_sql_LLM(payload: Dict[str, Any], catalog, id_cliente) -> Tuple[str, str]:
    ...
    try:
        raw = _call_ollama_with_retry(prompt)
        sql_generated = _clean_llm_output(raw)
        sql_final = (preamble + "\n" + sql_generated).strip() if preamble else sql_generated
    except TimeoutError:
        print("[LLM] Ollama timeout even after retry. Using deterministic fallback", flush=True)
    ...
    if not vr["ok"]:
        ...
        try:
            raw2 = _call_ollama_with_retry(fix_prompt)
