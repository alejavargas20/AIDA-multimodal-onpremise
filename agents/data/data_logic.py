...
def process(payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    """
    try:
        print("\n[DATA AGENT] Ha entrado bien en el agente de data", flush=True)
        
        CATALOG_PATH = "agents/data/catalog.json"
        
        import os
        if not os.path.exists(CATALOG_PATH):
            raise FileNotFoundError(f"El archivo de catálogo no existe en: {CATALOG_PATH}")
            
        catalog = Catalog.from_json(CATALOG_PATH)
        print("[DATA AGENT] Catalogo creado correctamente", flush=True)

        # modo por defecto para que siempre exista
        mode = "scalar"

        # IdCliente (si aplica)
        print("[DATA AGENT] Extracción de clientes", flush=True)
        params = payload.get("metadata", {}) or {}
        id_cliente = _extract_id_cliente(params)

        try:
            print("[DATA AGENT] Entra por Logica programada", flush=True)
            # Logica Programada
            sql = create_sql(payload, catalog, id_cliente)
            # ejecuta aquí para poder evaluar si vino vacío
            mode = _guess_mode(sql, default="scalar")
            execution_result = execute_sql(sql, mode=mode)
            print("\n[PROG] SQL: "+sql, flush=True)

        except Exception as e:
            print(f"[DATA AGENT] Falló Lógica Programada ({type(e).__name__}: {e})", flush=True)
            # LLM
            print("[DATA AGENT] Entra por LLM (Qwen)", flush=True)
            sql, mode = create_sql_LLM(payload, catalog, id_cliente)
            mode = _guess_mode(sql, default="scalar")
        ...
        print(f"\n[ERROR FATAL EN DATA AGENT]:\n{error_trace}\n", flush=True)
        
        return {
            "status": "error",
            "error": f"Crash interno en Data Agent: {str(fatal_error)}",
            "sql": None,
