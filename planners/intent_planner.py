...
from schemas.planning import QueryPlan

from utils.time_resolver import ultimo_mes_declaracion, ultimos_n_meses_where
from utils.join_recipes import (
...
def _detect_result_shape(prompt_lower: str) -> str:
    ...
    """
    if "muéstr" in prompt_lower or "muestr" in prompt_lower or "lista" in prompt_lower or "top " in prompt_lower:
        return "detail"

    if "por mes" in prompt_lower or "últimos" in prompt_lower or "ultimos" in prompt_lower or "evolución" in prompt_lower or "evolucion" in prompt_lower:
        return "series"

    if "por producto" in prompt_lower or "por oficina" in prompt_lower or "por región" in prompt_lower or "por region" in prompt_lower or "por tipo" in prompt_lower or "por " in prompt_lower:
        return "grouped"
